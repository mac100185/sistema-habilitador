...
$(document).ready(function () {
    ...
    $("#squadHabi").on('change', function () {
        ...
        comboIniciativa.innerHTML = '<option value="0">Seleccione una Iniciativa ...</option>';
        ...
        if (squad == "0") {
            ...
            vulnerabilidadInf.innerHTML = '<option value="0">Seleccione una Iniciativa ...</option>';
            ...
            fetch(host + "/api/seguridad_def/combo/squads/iniciativa/" + squad)
                ...
                .then(function (data) {
                    ...
                    if (iniciativa_ == "") {
                        ...
                        iniciaInf.innerHTML = '<option value="0">Sellecione un Iniciativa ...</option>';
                    } else {
    ...
    $('#mCEvidenciaHabSquad').summernote({
        ...
        //lang: 'ko-KR',
        height: 625,
        toolbar: [['style', ['irA']], ['font', ['style', 'bold', 'italic', 'underline', 'botonParrafo', 'clear', 'add-text-tags', 'superscript', 'subscript']], ['color', ['color']], ['para', ['ul', 'ol', 'paragraph']], ['table', ['table']], ['insert', ['botSeguDefTab', 'link', 'picture', 'iframe', 'botonShellLi', 'botonTablaUni']], ['view', ['fullscreen', 'codeview', 'help']]],
        buttons: {
            bto_Herra_Uni,
            bto_Servi_Uni,
            kali: botonKali,
    ...
    $(document).on("click", ".btnPlantillaHabi", function () {
        ...
        fetch(host + "/api/seguridad_def/controles_squad/unitario/squad/uni/" + IdHabSquad)
            .then(function (response) {
                return response.text();
            })
            .then(function (data) {
...
function renderDatatableHabi(id) {
    ...
    let tablaComando = $("#HabilitaSquad").DataTable({
        ...
        columns: [{
                ...
                render: function (data) {
                    data = ` ${data}`;
                    return '<span class="btnMostrarCon btnPlantillaHabi" style="white-space:normal; width:200px !important;" title="Mostrar Formato y Evidencia">' + data + "</span>";
                }
                  }, {
                data: "EstadoHabSquad",
                width: "20%",
        ...
        buttons: {
            ...
            buttons: [{
                ...
                filename: "Habilitadores de Seguridad Squad_" + SquadProy + "_" + time,
                title: "Habilitadores de Seguridad Squad: " + SquadProy + " Iniciativa: " + iniciativa,
...
$(document).on("click", ".btnEditarHabSquad", function () {
    ...
    fetch(host + "/api/seguridad_def/controles_squad/unitario/squad/uni/" + IdControl)
        .then(function (response) {
            return response.text();
        })
        .then(function (data) {
            ...
            $('#mCEvidenciaHabSquad').summernote("editor.pasteHTML", datos.EvidenciaHabSquad);
        });
        ...
        .then(function (data) {
            ...
            Highcharts.chart('container', {
                ...
                plotOptions: {
                    ...
                    pie: {
                        ...
                        dataLabels: {
                            useHTML: true,
                            enabled: true,
                            format: '<div class="formatoTextGra"> <span style="color:{point.color};">🞑 </span>{point.name}: {point.y:.0f}</div><div class="formatoTextGraPorce">{point.percentage:.1f} %</div>'
                        },
                        colors: ['#34ace0', '#706fd3', '#ff793f', '#33d9b2', '#d1ccc0', '#ff5252', '#ffda79']
                    }
                },
                series: {
                    borderRadius: 5,
                ...
                tooltip: {
                    ...
                    pointFormat: '<div style="color:{point.color}; text-decoration:none;">{point.name}</div>: <b style="text-decoration:none;">{point.y:.0f}</b> of total<br/>'
                },
                series: [{
                    name: 'Cumplimiento',
                    colorByPoint: true,
...
function graficaHabilImpre(Id) {
    let squad = $("#squadHabi option:selected").text();
    let iniciativa = $("#Hab_iniciativa option:selected").text();
    fetch(host + "/api/seguridad_def/controles_squad/grafica/lite/imprescindible/" + Id)
        .then(function (response) {
            return response.text();
        })
        .then(function (data) {
            ...
            Highcharts.chart('container2', {
                ...
                tooltip: {
                    shared: true,
                    useHTML: true,
                    headerFormat: '<div style="font-size:14px; text-decoration:none !important;">{series.name}</div><br>',
                    pointFormat: '<div style="text-decoration:none !important;">{point.name}</div>: <b style="text-decoration:none;">{point.y:.0f}</b> of total<br/>'
                },
