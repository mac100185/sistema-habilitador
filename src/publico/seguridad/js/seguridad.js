...
$(document).ready(function () {
      ...
      if (titulosSubtitulos == "") {
        ...
        for (var i = 0; i < titulosSubtitulos.length; i++) {
          arrayTemporal = nuevoArray.filter(
            (resp) => resp["Titulo"] == titulosSubtitulos[i]["NombrePrin"],
          );
      ...
      $(".dropdown-btn").dblclick(function () {
        ...
        fetch(host + "/api/seguridad_def/principal/" + id)
          ...
          .then(function (data) {
            ...
            document.getElementById("hDetallePri").innerHTML =
              datos.DetallePrin;
  ...
  $("#mEnlaceIntPri").summernote({
    ...
    height: 200,
    toolbar: [
      ["mybutton", ["botonEnlacePri", "enlace", "subEnlace"]],
      ["style", ["style"]],
      ["font", ["bold", "clear"]],
      ["color", ["color"]],
...
function mostrarSecundario(IdSecu) {
  fetch(host + "/api/seguridad_def/secundario/" + IdSecu)
    ...
    .then(function (data) {
      $("#hEnlaceIntPri").html(
        '<ul class="right-navegador" id="right-navegador-box"></ul>',
      );
      let datos = JSON.parse(data);
      ...
      $("#hDetallePri").html(
        '<header id="header"><hr></header><div id="article-content" class="markdown-body">' +
          datos.DetalleSecu +
          "</div>",
      );
