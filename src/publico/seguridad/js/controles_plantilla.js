...
$(document).ready(function () {
    ...
    $('#mCNombre').summernote({
        ...
        tabsize: 2,
        height: 300,
        toolbar: [['style', ['style']], ['font', ['bold', 'underline', 'clear']], ['color', ['color']], ['para', ['ul', 'ol', 'paragraph']], ['view', ['fullscreen', 'codeview']]]
    });
    ...
    $(document).on("click", ".btnEditar", function () {
        ...
        fetch(host + "/api/seguridad_def/controles/unico/" + IdControl)
            ...
            .then(function (data) {
                ...
                $('#mCCriterios_Cumpli').summernote("editor.pasteHTML", datos.Criterios_Cumpli);
                $('#mCAcciones_Comenta').summernote("editor.pasteHTML", datos.Acciones_Comenta);
                ...
                $('#mCModelo_Evidencia').summernote("editor.pasteHTML", datos.Modelo_Evidencia);
    ...
    fetch(host + "/api/seguridad_def/controles/unico/" + id)
        ...
        .then(function (datos) {
            ...
            if (datos.Acciones_Comenta.length > 2) {
                $('#comentariosRender').html(`<div class="tituloContr" style="font-size: 14px;">Comentarios</div><div class="detalleContr">${datos.Acciones_Comenta}</div>`);
            } else {
                $('#comentariosRender').html("");
            }

            if (datos.Cambios.length > 2) {
                $('#cambiosRender').html(`<div class="tituloContr" style="font-size: 14px;">Cambios</div><div class="detalleContr">${datos.Cambios}</div>`);
            } else {
                $('#cambiosRender').html("");
            }
            let imprecin;
            ...
            if (datos.Lineamiento.length > 2) {
                $('#lineamientoRender').html(`<div class="tituloContr" style="font-size: 14px;">Lineamiento</div><div class="detalleContr">${datos.Lineamiento}</div>`);
            } else {
            ...
            jQuery(function ($) {
                ...
                $("svg").each(function () {
                    ...
                    $(this).find(".textoDondeIn").css({
                        fill: '#' + textoInicio
                    });
                    $(this).find(".textoDondeCe").css({
                        fill: '#' + textoCerti
                    });
                    $(this).find(".textoDondePr").css({
                        fill: '#' + textoProd
                    });
        ...
        columns: [
            {
                ...
                render: function (data) {
                    data = ` ${data}`;
                    return '<span class="btnMostrarCon" style="white-space:normal; width:200px !important;">' + data + "</span>";
                }
                  }, {
